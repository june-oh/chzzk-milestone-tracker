...
export type ManualHistoryPoint = {
  date: string;
  followers: number;
};
...
/** Weekly follower trend from Softcon Viewership (2026-06-02 snapshot). */
const ARISA_MANUAL_FOLLOWER_HISTORY: ManualHistoryPoint[] = [
...
const MANUAL_FOLLOWER_HISTORY: Record<string, ManualHistoryPoint[]> = {
  "4de764d9dad3b25602284be6db3ac647": ARISA_MANUAL_FOLLOWER_HISTORY,
};
...
export function getManualFollowerHistory(channelId: string): ManualHistoryPoint[] {
  return MANUAL_FOLLOWER_HISTORY[channelId] || [];
}
...
export function enrichStreamer<T extends EnrichableStreamer>(streamer: T): T {
  ...
  const mergedHistory =
    manualFollowers.length > 0 || historyByDate.size > 0
